...
_What is necessary is that `predict.sh` generates predictions according to spec_.
...
docker run --rm -v $PWD/src:/job/src -v $PWD/work:/job/work -v <path_to_test_data>:/job/data -v $PWD/output:/job/output proj/demo bash /job/src/predict.sh /job/data/input.txt /job/output/pred.txt
```
